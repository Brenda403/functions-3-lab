...
// CODE HERE
const postTaxPrices = prices.map((tax) => tax * 0.07 + tax);
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter((combat) => combat.CP >= 200);
...
const priceTotal = orders.map((taxed) => taxed.price + taxed.price * taxed.tax);
...
const whereIsBob = purchases.filter((bob) => bob.owner === "Bob");
console.log(whereIsBob);
const bobsTotal = whereIsBob.reduce((a, c) => a + c.price, 0);
