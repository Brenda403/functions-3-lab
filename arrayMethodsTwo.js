...
const divideByTwo = numbers.forEach((el) => console.log(el / 2));
...
const makeACopy = numbers.map((num) => num / 2);

console.log(makeACopy);
...
// CODE HERE
const endsWithI = cities.filter((el) => el.charAt(el.length - 1) === "i");
...
const checkFruits = fruits.forEach((el, i) => {
  if (el.color === "red") {
    console.log(`The fruit with the index ${i} is an apple`);
  } else if (el.color === "orange") {
    console.log(`The fruit with index ${i} is an orange`);
  } else {
    console.log(`The fruit with the index ${i} is neither apple or orange`);
  }
});
...
/*
The fruit with the index 0 is neither apple or orange
The fruit with index 1 is an orange
The fruit with the index 2 is neither apple or orange
...
const foodsWithRice = foods.filter((food) => food.tags.includes("rice"));
const totalOfRice = foodsWithRice.reduce((a, c) => a + c.price, 0);

console.log(totalOfRice);
// THE TOTAL
